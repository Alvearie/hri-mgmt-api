...
then
  ...
  ibmcloud doi publishbuildrecord --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --branch $BRANCH_NAME --repositoryurl $github.event.repository.name --commitid $GITHUB_SHA --status pass

elif [ "$PUBLISH_TYPE" == "deployRecord" ]
then
  # Upload a deployment record; It is assumed that the deployment was successful
  ibmcloud doi publishdeployrecord --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --env=dev --status=pass

elif [ "$PUBLISH_TYPE" == "unitTest" ]
then
  # Upload unittest test record for the build
  ifFileExists "unittest.xml"
  ibmcloud doi publishtestrecord --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --filelocation=unittest.xml --type=unittest
...
then
  ...
  ibmcloud doi publishtestrecord --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --filelocation=ivttest.xml --type=ivt

elif [ "$PUBLISH_TYPE" == "dreddTest" ]
then
  # Upload Dredd test record as IVT for the build
  ifFileExists "dreddtests.xml"
  ibmcloud doi publishtestrecord --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --filelocation=dreddtests.xml --type=ivt

elif [ "$PUBLISH_TYPE" == "fvtTest" ]
then
  # Upload FVT test record for the build
  ifFileExists "fvttest.xml"
  ibmcloud doi publishtestrecord --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --filelocation=fvttest.xml --type=fvt
...
then
  ...
  ibmcloud doi publishtestrecord --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --filelocation=smoketests.xml --type=smoketests
...
then
  ...
  ibmcloud doi publishtestrecord --sqtoken="$SONARQUBE_CREDENTIALS" --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --filelocation=$REPORT_TASK_PATH --type=sonarqube
  find . -name '*.json'
...
then
  ...
  ibmcloud doi evaluategate --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --policy=WFFH-CI

elif [ "$PUBLISH_TYPE" == "evaluateCD" ]
then
  # Invoke a DevOps Insights gate to evaluated a policy based on uploaded data
  ibmcloud doi evaluategate --logicalappname="$MY_APP_NAME" --buildnumber="$GITHUB_RUN_NUMBER" --policy=WFFH-CD
fi