...
func TestGetBatch(t *testing.T) {
	...
	mt.Run("success", func(mt *mtest.T) {
		...
		roles := []string{auth.HriConsumer, auth.DataConsumer, auth.DataIntegrator, auth.DataInternal, auth.HriIntegrator, "hri_tenant_134340_data_integrator", "hri_tenant_134340_data_consumer"}

		tenantId := test.ValidTenantId
		//batchId := test.ValidBatchId
		claim := auth.HriAzClaims{Scope: auth.HriConsumer, Roles: roles, Subject: subject}
		params := model.GetBatch{
			TenantId: tenantId,
		}
		...
		tenants, response := GetBatch(requestId, params, claim)
	...
	mt.Run("No Auth", func(mt *mtest.T) {
		mongoApi.HriCollection = mt.Coll
		tenantId1 := "test-batches"
		tenantId2 := "provider1234-batches"
		tenantId3 := "pentest-batches"
		...
		tenants, response := GetBatchNoAuth(requestId, params, claim)
		tenenatsIdList := []model.GetTenantId{}
	...
	mt.Run("Invalied Claims", func(mt *mtest.T) {
		mongoApi.HriCollection = mt.Coll
		tenantId1 := "test-batches"
		tenantId2 := "provider1234-batches"
		tenantId3 := "pentest-batches"
	...
	mt.Run("Batch Not found", func(mt *mtest.T) {
		mongoApi.HriCollection = mt.Coll
		...
		result := model.GetBatchTenantDetail{
			TenantId: "",
		}
		tenant1 := mtest.CreateCursorResponse(1, "foo.bar", mtest.FirstBatch, bson.D{
