...
class HRIDeployHelper
  ...
  Logger.new(STDOUT).info("exe_path: , #{exe_path}")
    Open3.popen3("#{exe_path} -config-path=#{config_path} #{override_params} -kafka-properties=security.protocol:sasl_ssl,sasl.mechanism:PLAIN,sasl.username:token,sasl.password:#{ENV['KAFKA_PASSWORD']},ssl.endpoint.identification.algorithm:http 2> #{log_path}/#{log_prefix}error.txt > #{log_path}/#{log_prefix}output.txt &")
    sleep 1
